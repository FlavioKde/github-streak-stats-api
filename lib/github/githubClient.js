...
import { mapGitHubContributions, mapGitHubLanguages, mapGitHubActivity } from "./githubMapper.js";
import { getUserContributionQuery, getUserCreatedAtQuery, getUserLanguagesQuery, getUserActivityQuery } from "./githubQueries.js";
...
async function fetchSingleRange(username, fromDate, toDate) {
  ...
  try {
    ...
    console.log("DEBUG weeks:", userData.contributionsCollection?.contributionCalendar?.weeks);

    handleMissingUserData(userData, username);
console.log(userData.contributionsCollection?.contributionCalendar?.weeks)
...
export async function fetchUserLanguages(username) {
  ...
  const query = getUserLanguagesQuery;
  ...
  try {
    ...
    return user.repositories.nodes.map(repo => ({
      name: repo.name,
      languages: mapGitHubLanguages(repo.languages)
    }));
    ...
    throw new GithubApiError('Failed to fetch user languages from GitHub API', error);
  }
}
...
export async function fetchUserActivity(username) {
  ...
  const query = getUserActivityQuery;
  ...
  return mapGitHubActivity(user);
    ...
    throw new GithubApiError('Failed to fetch user activity from GitHub API', error);
  }
}
